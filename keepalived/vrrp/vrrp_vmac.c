...
/* makes a vmac on any interface */
/* */
bool
netlink_link_add_vmac_to_base( vrrp_t *vrrp, interface_t* base_ifp, char* vmac_ifname )
{
   ...
   */
  ifp = if_get_by_ifname(vmac_ifname, IF_CREATE_ALWAYS);
  ...
  ifp->is_ours = true;
  //Create required and the base interface exsists
  if (create_interface && base_ifp->ifindex) {
    ...
    addattr32(&req.n, sizeof(req), IFLA_LINK, base_ifp->ifindex);
    addattr_l(&req.n, sizeof(req), IFLA_IFNAME, vmac_ifname, strlen(vmac_ifname));
    ...
    if (base_ifp->vrf_master_ifp)
      addattr32(&req.n, sizeof(req), IFLA_MASTER, base_ifp->vrf_master_ifp->ifindex);
#endif

    if (netlink_talk(&nl_cmd, &req.n) < 0) {
      log_message(LOG_INFO, "(%s): Unable to create VMAC interface %s"
              , vrrp->iname, vmac_ifname);
      return false;
    }
     ...
     */
    netlink_interface_lookup(vmac_ifname);
  ...
  if (vrrp->family == AF_INET6 || vrrp->evip_other_family) {
    ...
    req.n.nlmsg_type = RTM_NEWLINK;
    req.ifi.ifi_family = AF_UNSPEC;
    req.ifi.ifi_index = (int)ifp->ifindex;
...
netlink_link_add_vmac(vrrp_t *vrrp)
{
  ...
  if( ! netlink_link_add_vmac_to_base( vrrp, vrrp->configured_ifp, vrrp->vmac_ifname ) )
    return false;
