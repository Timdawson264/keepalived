...
void
alloc_ipaddress(vrrp_t *vrrp, list ip_list, const vector_t *strvec, const interface_t *ifp, bool allow_track_group)
{
 ...
 */
  ...
  while (i < vector_size(strvec)) {
    ...
    if (!strcmp(str, "dev")) {
      ...
      //set the new addresses ifp to the devices specified after the Dev config.
      new->ifp = ifp_local;
...
//IF VMAC
  if( new->use_vmac  && new->ifp->if_type != IF_TYPE_MACVLAN ) {
    if( vrrp == NULL || new->ifp == NULL ) {
      report_config_error(CONFIG_GENERAL_ERROR, "use_vmac on virtual ip requires dev");
      return;
    }

    char vmac_ifname[IFNAMSIZ];
    snprintf( vmac_ifname, IFNAMSIZ, "%s.%s", vrrp->vmac_ifname, new->ifp->ifname );

    log_message(LOG_INFO, "(%s): creating VMAC interface %s", vrrp->iname, vmac_ifname);

    netlink_link_add_vmac_to_base( vrrp, new->ifp, (char *)vmac_ifname );

    if ( ! (new->ifp = if_get_by_ifname(vmac_ifname, IF_NO_CREATE )) ) {
      report_config_error(CONFIG_GENERAL_ERROR, "vmac device not created: %s", vmac_ifname);
      return;
    }
  }
